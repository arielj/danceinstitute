...
class MovementTests(custom_test_case.CustomTestCase):
  ...
  def test_incoming_returns_only_incoming_movements(self):
    ...
    incoming = map(lambda m: m.id, movement.Movement.incoming())
    self.assertIn(mv1.id,incoming)
    self.assertNotIn(mv2.id,incoming)
    self.assertNotIn(mv3.id,incoming)
    self.assertIn(mv4.id,incoming)
  ...
  def test_incoming_returns_only_incoming_movements(self):
    ...
    mv3 = Factory.create('movement', {'direction': 'out'})
    mv4 = Factory.create('movement', {'direction': 'in'})
    outgoing = map(lambda m: m.id, movement.Movement.outgoing())
    self.assertNotIn(mv1.id,outgoing)
    self.assertIn(mv2.id,outgoing)
    self.assertIn(mv3.id,outgoing)
    self.assertNotIn(mv4.id,outgoing)
